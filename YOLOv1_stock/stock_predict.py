...
def decoder(pred):
    ...
    # 如果boxes为0，表示没有框，返回0
    if len(boxes) == 0:
        # print(f"{image_name}: There is no box!")
...
# 5. 预测函数
def predict_single(model, image_name, root_path='', cal_mAP=False):
    result = []  # 保存结果的变量
    # 打开图片
    ...
    for i, box in enumerate(boxes):
        ...
        # 把这些值集中放入一个变量中返回
        if not cal_mAP:
            result.append([(x1, y1), (x2, y2), STOCK_CLASSES[cls_index], image_name, prob])
        else:
            image_name = image_name.split('/')[-1]
            result.append([image_name, cls_index, prob, x1, y1, x2, y2])
    return result


def calculate_map(model, dataset_path, annotation_path):
    pred_bboxes = []
    image_name_list = []
    ...
    with open(annotation_path, 'r') as f:
        for line in f.readlines():
            ...
            image_name = data[0]
            image_name_list.append(os.path.join(dataset_path, image_name))

    for image_name in image_name_list:
        result = predict_single(model, image_name, cal_mAP=True)
        pred_bboxes.extend(result)
    ...
    with open(annotation_path, 'r') as f:
        for line in f.readlines():
            ...
            for i in range(num_boxes):
                ...
                x2 = int(data[3 + i * 5])
                y2 = int(data[4 + i * 5])
                class_pred = int(data[5 + i * 5])
                true_bboxes.append([image_name, class_pred, 1, x1, y1, x2, y2])

    mAP = mean_average_precision(pred_bboxes, true_bboxes, iou_threshold=0.5, num_classes=3)
    return mAP
...
if __name__ == '__main__':
    ...
    model = resnet50()
    print('load model...')
    model.load_state_dict(torch.load('./save_weights/weight_May01_22-47-46.pth'))
    # model.load_state_dict(torch.load('./save_weights/yolo_stock_4_6.pth'))
    ...
    if not cal_mAP:
        # 设置图片路径
        base_path = 'dataset_midterm/test_images/stock_test_images/'
        # base_path = './stock_test_images_new/'
        image_name_list = [base_path + i for i in os.listdir(base_path)]
    else:
        base_path = 'dataset_midterm/Dataset_stock/JPEGImages_train_2024_4_6/'
        annotation_path = 'utils/stocktest.txt'
    # 打乱顺序
    ...
    if not cal_mAP:
        ...
        for image_name in image_name_list:
            ...
            cv2.imwrite(f'dataset_midterm/test_images/stock_test_images_results/result_{base_image_name}', image)
            # cv2.imwrite(f'./stock_test_images_new_results/result_{base_image_name}', image)
    else:
        print('calculating mAP....')
        mAP = calculate_map(model, base_path, annotation_path)
        print(f'mAP:{mAP}')
