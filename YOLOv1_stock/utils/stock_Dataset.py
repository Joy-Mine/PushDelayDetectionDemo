...
class Yolo_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 需要特别处理，将各种信息变为yolov1需要的7*7*13
        target = self.encoder(boxes, labels)  # 7x7x13
        # 最后，进行预处理操作
    ...
    def encoder(self,boxes,labels):
        ...
        target = torch.zeros((grid_num,grid_num,13))
        # 缩放因子
...
def main():
    ...
    file_root = '../../dataset_stock/JPEGImages_train_2024_4_6' # 记得改为自己的路径
    train_dataset = Yolo_Dataset(root=file_root,list_file='stocktrain.txt',train=True,transforms = [T.ToTensor()] )
